...
import './services/envDiagnostic'; // Run diagnostic on load
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    runConnectionTests().then(result => {
      if (result.allPassed) {
        ...
        console.warn('⚠️ Some Supabase connections failed. Cloud features may be limited.');
      }
    });
  ...
  const handleDataImport = (data: Partial<ResumeData>) => {
    ...
    if (data.personalInfo) {
      // If importing from DB, we might want to keep the ID or generate new.
      // If we keep ID, we might overwrite local changes to that same ID. 
      // For safety in this "Builder" app, let's treat imports as new local copies unless ID logic is strict.
      // However, if we want to "Edit" the cloud version, we should probably keep the ID.
      // Let's assume we overwrite local with cloud data if ID matches, or create new.

      const importedResume = {
        ...createNewResume(),
        ...data,
        // If the data has an ID, use it, otherwise generate. 
        // Warning: reusing ID means local overwrite.
