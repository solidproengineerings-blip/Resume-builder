...
export const EditorView: React.FC<EditorViewProps> = ({ initialData, onSave, onBack }) => {
  ...
  const handleCloudSave = async () => {
    ...
    try {
        ...
        if (previewRef.current) {
             // We need to capture the element. Since it might be scaled in UI, 
             // html2canvas usually handles what is rendered. 
             // Ideally we capture it at scale 1, but for simplicity we capture the container.
             // We target the inner 'a4-paper' div via querySelector or assume previewRef is the wrapper.
             const elementToCapture = previewRef.current.querySelector('.a4-paper') as HTMLElement;
        ...
        <div className="max-w-full mx-auto flex justify-between items-center gap-4">
          <div className="flex items-center gap-3 flex-1">
             ...
             <div className="flex flex-col">
                <input 
                  ...
                  className="font-bold text-gray-900 bg-transparent hover:bg-gray-50 border border-transparent hover:border-gray-200 rounded px-1.5 py-0.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all w-48 sm:w-64"
                />
                <div className="flex items-center gap-2 px-1.5">
                   <span className="text-[10px] uppercase font-bold text-blue-600 tracking-wider">Editor</span>
                   {isSaving ? (
                     <span className="text-[10px] text-gray-400 flex items-center gap-1">Saving...</span>
                   ) : (
                     <span className="text-[10px] text-green-600 flex items-center gap-1">Saved Locally</span>
                   )}
                </div>
             </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {supabase && (
                ...
                <span className="hidden sm:inline">{isCloudSaving ? 'Uploading...' : 'Save to Cloud'}</span>
                </button>
            )}
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        <div className="w-full lg:w-1/2 h-full overflow-y-auto custom-scrollbar bg-white border-r border-gray-200 no-print flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10">
          <div className="p-6 md:p-8 pb-32">
            <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Resume Details</h2>
                <p className="text-gray-500 text-sm">Fill in the sections below. Use the AI wand to auto-generate content.</p>
            </div>
